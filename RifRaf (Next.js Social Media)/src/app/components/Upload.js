...
const Upload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const convertedFilePath = data.outputPath;

      // Read the converted file
      const convertedFile = await fetch(convertedFilePath).then(res => res.blob());

      const uploadParams = {
        Bucket: 'rifraf.site',
        Key: `videos/${uploadName}`,
        Body: convertedFile,
        ContentType: 'video',
      };
      ...
      const signedUrl = await getSignedUrl(s3Client, command, { expiresIn: 600000 }); // 1 Week
  ...
  const streamToString = async (stream) => {
    ...
    result += decoder.decode(); // end the decoding stream
    return result;
  };  
